...
把当前“六模型高密度审计报告”改造成两层产品：面向初学者的概念递进课程，以及面向工程师的六模型证据化参考手册。第一优先级是修正计算口径和源码/部署实现混淆；第二优先级是交互可视化、信息层级、移动端与无障碍。保持 GitHub Pages 静态部署，不引入前端框架或运行时第三方依赖。
...
4. **移除运行时 Mermaid**：以语义 HTML、CSS 和轻量 SVG 绘制宏观图，避免第三方阻塞、隐藏容器重绘和失败空白。
5. **课程与参考分离**：概念课程先讲心智模型，六模型详情放到案例手册，复杂实现使用原生 `<details>` 渐进披露。
...
- 模型参考卡显示官方/源码/推导徽标和可打开的具体来源。
- 图不只依赖颜色；宏观图核心节点不超过 9 个。

### 可访问性与渐进增强

- 禁用 JavaScript 后课程和六模型参考内容仍可阅读。
...
- JavaScript 总量目标 < 50KB、CSS < 60KB（未压缩），无大型图片和字体。
- 同一交互只绑定一次，不做全页重复渲染。
...
**验收标准**
- 六章学习路径和六模型参考均在无 JS 情况可读。
- 页面包含跳转链接、语义地标、来源区和理解检查。
- 不再加载 Mermaid CDN。
